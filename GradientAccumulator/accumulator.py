...
class OldGradientAccumulator(tf.keras.optimizers.Optimizer):
    ...
    def __init__(
        self,
        optimizer,  # removed typehint!
        accum_steps: types.TensorLike = 4,
        name: str = "OldGradientAccumulator",
        **kwargs,
    ):
        r"""Construct a new OldGradientAccumulator optimizer.
        ...
        Args:
            ...
            accum_steps: int > 0. Update gradient in every accumulation steps.
            name: Optional name for the operations created when applying
                gradients. Defaults to "OldGradientAccumulator".
            **kwargs: keyword arguments. Allowed to be {`clipnorm`,
                `clipvalue`, `lr`, `decay`}. `clipnorm` is clip gradients by
                norm; `clipvalue` is clip gradients by value, `decay` is
                included for backward compatibility to allow time inverse
        ...
        self.optimizer = optimizer  # added this!
    ...
    def get_config(self):
        config = {"optimizer": self.optimizer, "accum_steps": self._accum_steps}  # updated this to add optimizer argument
        base_config = super().get_config()
        return {**base_config, **config}
